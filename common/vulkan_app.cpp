...
std::vector<char> VulkanApp::readFile(const std::string &filename)
{
    ...
    if (!file.is_open())
    {
        ...
        std::vector<std::string> possiblePaths = {
            getShaderDir() + filename, // Relative to executable
            "./shaders/" + filename,   // Relative to cwd
        };
        ...
        for (const auto &path : possiblePaths)
        {
            ...
            if (file.is_open())
            {
                std::cout << "Successfully opened shader file at: " << path << std::endl;
                break;
            }
        }
        ...
        if (!file.is_open())
        {
            throw std::runtime_error("Failed to open file: " + filename +
                                     ". Tried multiple paths. Current working directory: " +
...
#pragma warning(disable : 4996) // Disable warning about using tmpnam
    std::string tempFilename = std::tmpnam(nullptr);
    tempFilename += ".spv";

    // Determine shader type flag
    std::string shaderTypeFlag;
    switch (shaderStage)
    {
    case VK_SHADER_STAGE_VERTEX_BIT:
        shaderTypeFlag = "-fshader-stage=vertex";
        break;
    ...
    // Write shader source to a temporary file
    std::string sourceFilename = std::tmpnam(nullptr);
    ...
    // Read the compiled shader
    std::vector<char> compiledShader = readFile(tempFilename);
...
// Debug messenger setup and cleanup functions
...
void VulkanApp::initWindow()
{
    ...
    window = glfwCreateWindow(windowWidth, height, appName.c_str(), nullptr, nullptr);
...
void VulkanApp::createGraphicsPipeline()
{
    ...
    catch (const std::exception &e)
    {
        ...
        try
        {
            vertShaderCode = readFile("shader.vert");
            fragShaderCode = readFile("shader.frag");
...
void VulkanApp::recordRenderCommands(VkCommandBuffer commandBuffer)
{
    // Default implementation draws a single triangle
...
void VulkanApp::drawFrame()
{
    ...
    recordRenderCommands(commandBuffers[imageIndex]);
...
VulkanApp::QueueFamilyIndices VulkanApp::findQueueFamilies(VkPhysicalDevice dev)
{
...
VulkanApp::SwapChainSupportDetails VulkanApp::querySwapChainSupport(VkPhysicalDevice dev)
{
